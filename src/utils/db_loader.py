...
from impl_config import State, AgeGroup, Gender, SexualOrientation, PairingType, SystemPairing, PartnerCount
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as agents_file:
        ...
        for row in reader:
            agent_data = {
                ...
                'last_sti_test_date': datetime.strptime(row['last_sti_test_date'], '%Y-%m-%d'),
                ...
                'loc': row['loc'],
                'system_pairing': SystemPairing(row['system_pairing'])
            }
